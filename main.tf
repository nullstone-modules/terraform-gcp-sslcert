...
locals {
  domain_names    = { for key, _ in var.domains : key => data.google_dns_managed_zone.domains[key].dns_name }
  subdomain_names = { for key, _ in var.domains : key => key != "." ? "${key}.${local.domain_names[key]}" : local.domain_names[key] }
}
...
locals {
  auth_records = { for key, _ in var.domains : key => google_certificate_manager_dns_authorization.this[key].dns_resource_record.0 }
}
